...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String formUrl = basePath + "pages/back/task/TaskServletBack/listSplit" ;
%>
<html>
<head>
<base href="<%=basePath%>">
			...
			<table class="tablelist">
				...
				<tbody>
					<c:forEach items="${allTasks}" var="task">
						<tr>
							<td><input type="checkbox" id="tid" value="${task.tid}"></td>
							<td><c:if test="${allActions['31'] != null}">
									<a
										href="<%=basePath%>${allActions['31'].url}?cid=${task.client.cid}&tid=${task.tid}">${task.title}</a>
								</c:if> <c:if test="${allActions['31'] == null}">
									${task.title}
								</c:if></td>
							<td>
								<c:if test="${allActions['3'] != null}">
									<a href="<%=basePath%>${allActions['3'].url}?client.cid=${task.client.cid}" class="tablelink">${allClients[task.client.cid]}</a>
								</c:if>
								<c:if test="${allActions['3'] == null}">
							...
							<td>
								<c:if test="${allActions['7'] != null}">
									<a href="<%=basePath%>${allActions['7'].url}?tid=${task.tid}" class="tablelink">修改</a>
								</c:if>
								<c:if test="${task.status == 1}">
									<c:if test="${allActions['8'] != null}">
										<a href="<%=basePath%>${allActions['8'].url}?tid=${task.tid}" class="tablelink">完成</a>
									</c:if>
									<c:if test="${allActions['9'] != null}">
										<a href="<%=basePath%>${allActions['9'].url}?tid=${task.tid}" class="tablelink">关闭</a>
									</c:if>
								</c:if>
							</td>
						</tr>
